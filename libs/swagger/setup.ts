...
export function setupSwagger(
  ...
  if (options?.include) {
    ...
    const modules = scanner.getModules(((app as any).container as NestContainer).getModules(), options.include || []);
    const appConfig = (app as any).config as ApplicationConfig;

    modules.sort((a, b) => (options.include?.indexOf(a.metatype) || 0) - (options.include?.indexOf(b.metatype) || 0));
